...
class User_Registration 
{
   ...
   function getRole()
   {
        ...
        $out = DB::select("select * from app_role_master where is_active='Y' and id not in ('1','2','9','6') order by name");
   ...
   function maxUserMasterId()
   {
    ...
    function appUserMenuAccessMapId()
    {
        $out = DB::select("select COALESCE(max(id), 0) + 1 as max_id from app_user_menu_access_map");
        $result = json_decode(json_encode($out), true);
        foreach ($result as $key => $value) 
        {
            return $value['max_id'];
    ...
    function saveERPUser($first_name,$last_name,$offical_email,$offical_mobile,$personal_email,$personal_mobile,$monthly_target,$password_erp_user,$table_user_dept,$table_user_location,$table_user_menu,$table_user_access,$user_id)
    {
        ...
        if($totalAuthEmailCount==0)
        {
            ...
            $countTable = count($table_user_dept);

            foreach ($table_user_dept as $key_dept => $value_dept) { $user_dept[] = $value_dept['dept']; }

            foreach ($table_user_location as $key_location => $value_location) { $user_location[] = $value_location['location']; }

            foreach ($table_user_menu as $key_menu => $value_menu) { $user_menu[] = $value_menu['menu']; }

            foreach ($table_user_access as $key_access => $value_access) { $user_access[] = $value_access['access']; }

            for($i=0;$i<$countTable;$i++)
            {
                $role_user_dept = $user_dept[$i];
         
                $role_value_location = $user_location[$i];
            
                $role_value_menu = $user_menu[$i];
             
                $role_value_access = $user_access[$i];

                $appUserMenuAccessMapId = $this->appUserMenuAccessMapId();

                $out = DB::insert("insert into app_user_menu_access_map(id,user_id,menu_url_map_id,role_id,location_id,access,is_active,created_by,created_at) values('$appUserMenuAccessMapId','$maxUserMasterId','$role_value_menu','$role_user_dept','$role_value_location','$role_value_access','Y','$user_id','$this->created_at')");

                $out_2 = DB::insert("insert into app_user_role_map(user_id,role_id) values('$maxUserMasterId','$role_user_dept')");
            }
    ...
    function updateERPUser($erpuserid,$first_name,$last_name,$offical_mobile,$personal_email,$personal_mobile,$monthly_target,$password_erp_user,$table_user_dept,$table_user_location,$table_user_menu,$table_user_access,$updated_by)
    {
        ...
        DB::delete("delete from app_user_menu_access_map where user_id='$erpuserid'");
        ...
        for($i=0;$i<$countTable;$i++)
        {
            ...
            $out = DB::insert("insert into app_user_menu_access_map(id,user_id,menu_url_map_id,role_id,location_id,access,is_active,created_by,created_at) values('$appUserMenuAccessMapId','$erpuserid','$role_value_menu','$role_user_dept','$role_value_location','$role_value_access','Y','$updated_by','$this->created_at')");

            $out_2 = DB::insert("insert into app_user_role_map(user_id,role_id) values('$erpuserid','$role_user_dept')");
        }
    }
    ...
    function getRoleExceptAdmin()
    {
        $output = DB::select("select * from app_role_master where id not in ('1','2','3','4','6','9')  order by id");

        return json_decode(json_encode($output), true);
    }

    function getERPUserMenuAccessMap($erpUserId)
    {
        $output = DB::select("select * from app_user_menu_access_map where user_id='$erpUserId' and is_active='Y'");

        return json_decode(json_encode($output), true);
    }

    function inactiveERPUserStatus($erpUserMenuAccessMapID,$updated_by)
    {
        $out = DB::update("update app_user_menu_access_map set is_active='N',updated_by='$updated_by',updated_at='$this->created_at' where id='$erpUserMenuAccessMapID'");
